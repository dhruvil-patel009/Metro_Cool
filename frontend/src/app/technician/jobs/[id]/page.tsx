...
export default function JobDetailsPage() {
  ...
  const parseAddress = (address: any): Address | null => {
    ...
    if (typeof address === "string") {
      try { return JSON.parse(address) } catch { return null }
    }
    return address
  }
    ...
    const fetchBooking = async () => {
      try {
        ...
        if (json.success) {
          ...
          setJobStatus(json.booking.job_status)

          // if technician refresh during working
          if (json.booking.job_status === "working") {
            setIsWorking(true)
          }
        }
      } catch (err) {
  ...
  const markOnTheWay = async () => {
    ...
    try {
      ...
      await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/tech-jobs/${booking.id}/on-the-way`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  const startWork = async () => {
    ...
    try {
      ...
      await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/tech-jobs/${booking.id}/start-work`,
        {
          method: "PATCH",
          headers: { Authorization: `Bearer ${token}` },
        }
  ...
  if (!booking) return <div className="p-10 text-center">Job not found</div>
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      {/* HEADER */}
      <header className="bg-white border-b border-slate-200 relative top-0 z-50">
        ...
        >
          ...
          {jobStatus === "on_the_way" && (
            <Button
              disabled={updating}
              onClick={startWork}
              className="w-full lg:w-auto cursor-pointer bg-emerald-500 hover:bg-emerald-600 text-white font-black text-lg px-8 py-6 rounded-2xl shadow-lg flex items-center justify-center gap-3"
            >
              <Car className="w-6 h-6" />
