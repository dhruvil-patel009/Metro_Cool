...
export function JobList({ bookings, serverTime }: { bookings: any[]; serverTime: string }) {
  ...
  // Show today's jobs (including open ones available to accept)
  ...
  // If none today → show upcoming jobs (today or future) + open jobs, sorted by date asc.
  const baseList = useMemo(() => {
    if (todayJobs.length > 0) return todayJobs

    // Include all open jobs (they are available to accept regardless of date)
    // Plus any upcoming assigned/in-progress jobs
    const upcoming = [...bookings]
      .filter((j) => {
        const d = j.booking_date?.slice(0, 10) ?? ""
        // Show open jobs always, and future jobs that aren't completed
        return j.job_status === "open" || (d >= today && j.job_status !== "completed")
      })
      .sort((a, b) => {
        // Open jobs first, then sort by date
        if (a.job_status === "open" && b.job_status !== "open") return -1
        if (a.job_status !== "open" && b.job_status === "open") return 1
  ...
  return (
    <div className="space-y-4 mt-6">
      {/* Search + Filter bar */}
      <div className="bg-white p-3 sm:p-4 rounded-xl shadow-sm flex flex-col sm:flex-row gap-3 sm:gap-4">
        <div className="flex-1 relative group">
          <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within:text-blue-500 transition-colors" />
          <Input
            className="pl-12 bg-white border-slate-200 h-11 sm:h-12 text-sm focus-visible:ring-1 focus-visible:ring-blue-500 focus-visible:border-blue-500"
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button
              variant="outline"
              className="h-11 sm:h-12 px-4 sm:px-6 font-bold cursor-pointer border-slate-200 text-slate-700 min-w-[140px] justify-between bg-transparent"
            >
      ...
      {/* Job cards */}
      {filtered.length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm py-12 sm:py-16 flex flex-col items-center gap-3 text-slate-400">
