...
export function JobStepper({ currentStep }: { currentStep: number }) {
  const steps = [
    ...
    { icon: Settings, label: "Working", status: "active", subLabel: "In Progress" },
          ...
          <div key={step.label} className="flex items-center flex-1 last:flex-none">
            <div className="flex flex-col items-center gap-3 relative z-10 min-w-[100px]">
              <div
                className={cn(
                  ...
                  isCompleted && "bg-[#0891b2] border-white text-white shadow-lg shadow-cyan-100",
                  isActive && "bg-white border-[#0891b2] text-[#0891b2] shadow-xl shadow-cyan-50 ring-4 ring-cyan-50",
                  !isActive && !isCompleted && "bg-slate-50 border-white text-slate-200",
                )}
              >
                {typeof step.icon === "string" ? (
                  <span className="font-black text-xl">{step.icon}</span>
                ) : (
                  <step.icon className={cn("w-6 h-6", isActive && "animate-spin-slow")} />
                )}
              </div>
              <div className="text-center">
                ...
                {step.time && <p className="text-[10px] font-bold text-slate-400">{step.time}</p>}
                {step.subLabel && (
                  <p className="bg-[#0891b2]/10 text-[#0891b2] text-[8px] font-black px-1.5 py-0.5 rounded-full uppercase mt-1">
                    {step.subLabel}
                  </p>
                )}
              </div>
            ...
            {index < steps.length - 1 && (
              <div className="flex-1 h-[6px] bg-slate-100 rounded-full mx-2 -translate-y-6 relative overflow-hidden">
                <div
                  className={cn(
                    "absolute inset-0 bg-[#0891b2] transition-all duration-700",
