...
export default function BookingConfirmPage() {
  ...
  const handleCompleteBooking = async () => {
    try {
      ...
      router.push(`/services/${id}/booking/success`)
    } catch (err: any) {
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    {ISSUE_OPTIONS.map((issue) => (
                      <button
                        ...
                        className={`flex items-center justify-center cursor-pointer gap-2 px-4 py-3 rounded-lg border-2 text-sm font-semibold transition-all ${selectedIssues.includes(issue.id)
                    ...
                    {savedAddresses.map(addr => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left border rounded-lg p-4 transition ${selectedAddressId === addr.id
